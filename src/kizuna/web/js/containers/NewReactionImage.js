...
import { GoChevronLeft, GoCloudUpload } from 'react-icons/lib/go'
...
export class NewReactionImageContainer extends Component {
  ...
  doSetup() {
    ...
    fetch('/api/react/tags')
      ...
      .then(json => {
        this.setState({
          suggestions: _map(json.tags, 'name'),
  ...
  autocompleteRenderInput({ alreadyUsedTags }, { addTag, ...props }) {
    ...
    let suggestions = _difference(allSuggestions, alreadyUsedTags).filter(
      state => {
  ...
  render() {
    ...
    let body = (() => {
      if (hasImages) {
        if (uploading) {
          return (
            <div style={{ ...modal, background: '#f1f1f1' }}>
              <div style={{ ...innerModal }}>
                <h1 style={{ marginTop: '0' }}>
                  uploading your images<span className="loading">...</span>
                </h1>
                <div>
                  {images.map(image => {
                    return (
                      <img
                        ...
                        style={{
                          height: images.length > 10 ? '50px' : '100px',
                        }}
                      />
                    )
                  })}
          ...
          return (
            <div style={{ textAlign: 'center', padding: '5px' }}>
              ...
              <div style={{ marginBottom: '1em' }}>
                <p>
                  <a className="pure-button" href="/react">
                    Image Gallery
                  </a>
                  <span> </span>
                  <button
                    className="pure-button"
                    onClick={this.doSetup.bind(this)}
                  >
        ...
        return (
          <form onSubmit={this.uploadImages.bind(this)}>
            <Sticky enabled={true}>
              <div
                className="kiz-upload-menu"
                style={{ background: '#f1f1f1', padding: '5px' }}
              >
                <button
                  className="kiz-upload-menu__reset pure-button"
                  onClick={this.doSetup.bind(this)}
                >
            ...
            {_filter(images, { uploaded: false }).map(image => {
              ...
              return (
                <div key={image.id} style={rowStyle}>
                  {image.uploaded && <strong>Uploaded successfully</strong>}
                  {image.uploadFailure && (
                    <strong style={{ color: 'red' }}>
                      This Image Did Not Upload Successfully
                    </strong>
                  )}
                  <input
                    type="text"
                    ...
                    onChange={this.changeImageValue.bind(
                      this,
                      image.id,
                      'title'
                    )}
                  ...
                  <div>
                    <TagsInput
                      ...
                      renderInput={
                        image.uploaded === true
                          ? undefined
                          : this.autocompleteRenderInput.bind(this, {
                              alreadyUsedTags: image.tags,
                            })
                      }
          ...
          >
            <div style={dropzoneTextStyles}>
              ...
              <strong style={{ marginTop: '1em', display: 'block' }}>
                <button className="pure-button">browse</button>
                or drag images here.
              </strong>
            </div>
          </Dropzone>
