...
TRAVIS_COMMIT ?= $(shell git rev-parse HEAD)-ts$(shell date +"%T")
...
.PHONY: registry_pull_% image_% registry_push_% kube_deploy_% docker_clean
...
image_base:
	docker build \
		...
		--cache-from $(registry_prefix)/base \
		-t $(local_prefix)/base \
		.
...
image_%: image_base
	docker build \
		...
		--cache-from $(registry_prefix)/$(*) \
		-t $(local_prefix)/$(*) \
		.

image_web: image_base node_modules
	docker build \
		--file docker/web/Dockerfile \
		--cache-from $(registry_prefix)/web \
		-t $(local_prefix)/web \
		.
...
static/dist: node_modules webpack.config.js src/kizuna/api/web
